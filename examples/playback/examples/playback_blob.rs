...
use uncage_protos::pb::FrameSequence;
use uncage_protos::prost::bytes::BytesMut;
use uncage_protos::prost::{self, Message};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    while blob.read(&mut data).expect("Failed to read") != 0 {
        ...
        for frame in seq.frame {
            _patch += 1;
            let matches = patcher.apply_patch(frame.patch.into()).unwrap();
